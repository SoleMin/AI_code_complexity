...
def save_embedding(output_path, model, files, dimensions):
    ...
    out = []
    for f in files:
        identifier = f.split("/")[-1].strip(".java")
        print("id=",identifier)
...
def main(args):
    ...
    """
    input_path='data/CodeDataset/'
    output_path='features/nci2.csv'
    # graphs = glob.glob(input_path + "*.java")
    # print(graphs)
    temp_data = pd.read_csv('data/finalFeatureData.csv').dropna()
    data_name = temp_data['file_name'].reset_index(drop=True)

    graphs=[input_path+i for i in data_name]
